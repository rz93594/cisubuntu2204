...
deb_ensure_nftables_rules_permanent()
{
	...
	deb_ensure_nftables_rules_permanent_chk()
	{
      ...
      l_input_rule="" l_forward_rule="" l_output_rule="" l_rule_tst=""
		
      # Test for include
		if grep -Eqs "^\s*include" /etc/sysconfig/nftables.conf; then
      
         # Test for rules
         l_input_rule="$(awk '/hook input/,/}/' $(awk '$1 ~ /^\s*include/ { gsub("\"","",$2);print $2 }' /etc/sysconfig/nftables.conf))"
         l_forward_rule="$(awk '/hook forward/,/}/' $(awk '$1 ~ /^\s*include/ { gsub("\"","",$2);print $2 }' /etc/sysconfig/nftables.conf))"
         l_output_rule="$(awk '/hook output/,/}/' $(awk '$1 ~ /^\s*include/ { gsub("\"","",$2);print $2 }' /etc/sysconfig/nftables.conf))"
         ...
         echo -e "- NO include line found in /etc/sysconfig/nftables.conf" | tee -a "$LOG" 2>> "$ELOG"
      fi
	...
	deb_ensure_nftables_rules_permanent_fix()
	{
      ...
      echo -e "- Edit the /etc/sysconfig/nftables.conf file and un-comment or add a line with 'include \"<Absolute path to nftables rules file>\"' for each nftables file you want included in the nftables ruleset on boot." | tee -a "$LOG" 2>> "$ELOG"
