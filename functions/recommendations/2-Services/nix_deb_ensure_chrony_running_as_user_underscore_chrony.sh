...
deb_ensure_chrony_running_as_user_underscore_chrony()
{
	...
	deb_ensure_chrony_running_as_user_underscore_chrony_fix()
	{
		...
		l_fafile="$(grep -Pl -- "^\s*user" $l_searchloc)"
		...
		if [ -n "$l_fafile" ]; then
			...
			if ! grep -Pslq -- "^\h*user\h+_chrony" $l_searchloc; then 
				echo -e "- Setting \"user\" to \"_chrony\" in \"/etc/chrony/conf.d/chrony.conf\"" | tee -a "$LOG" 2>> "$ELOG"
				echo "user _chrony" >> /etc/chrony/conf.d/chrony.conf 
			fi
