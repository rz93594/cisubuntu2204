...
ensure_events_modify_date_time_information_collected()
{
	...
	{
        echo "- Start check - Ensure events that modify date and time information are collected" | tee -a "$LOG" 2>> "$ELOG"
		l_output="" l_output2=""

		# check the rule(s) for adjtimex,settimeofday,clock_settime
		...
		# check the rule(s) for /etc/localtime

		# Set the files and directories we're interested in
		l_rule_2="/etc/localtime"
		...
		if [ -z "$l_output2" ]; then
			echo -e "- PASS:\n$l_output\n" | tee -a "$LOG" 2>> "$ELOG"
			echo -e "- End check - Ensure events that modify date and time information are collected" | tee -a "$LOG" 2>> "$ELOG"
	...
	{
		echo "- Start remediation - Ensure events that modify date and time information are collected" | tee -a "$LOG" 2>> "$ELOG"

		if [ "$l_sysarch" = "b64" ]; then
			# Fix b64 rules with syscall adjtimex,settimeofday,clock_settime
			echo -e "- Fixing arch=b64 ondisk rules for adjtimex,settimeofday,clock_settime" | tee -a "$LOG" 2>> "$ELOG"
			fix_auditd_syscall_rule "arch=b64" "$l_rule_1" "$l_parameters_rule_1" "time-change" "/etc/audit/rules.d/50-time-change.rules"
		fi

		# Fix b32 rules with syscall adjtimex,settimeofday,clock_settime
		echo -e "- Fixing arch=b32 ondisk rules for adjtimex,settimeofday,clock_settime" | tee -a "$LOG" 2>> "$ELOG"
		fix_auditd_syscall_rule "arch=b32" "$l_rule_1" "$l_parameters_rule_1" "time-change" "/etc/audit/rules.d/50-time-change.rules"

		# Fix ondisk rules with files and directories
        echo -e "- Fixing ondisk rules for /etc/localtime" | tee -a "$LOG" 2>> "$ELOG"
        fix_auditd_watcher_rule "$l_rule_2" "$l_permissions_rule_2" "time-change" "/etc/audit/rules.d/50-time-change.rules"

		echo "- Reboot required to reload the active auditd configuration settings" | tee -a "$LOG" 2>> "$ELOG"
        echo "- End remediation - Ensure events that modify date and time information are collected" | tee -a "$LOG" 2>> "$ELOG"
		G_REBOOT_REQUIRED="yes"			
	}		
