...
echo -e "\n**************************************************\n- $(date +%d-%b-%Y' '%T)\n- Start Recommendation \"$RN - $RNA\"" | tee -a "$LOG" 2>> "$ELOG"
   ...
   {
      echo -e "- Start check - Ensure mounting of udf filesystems is disabled" | tee -a "$LOG" 2>> "$ELOG"
      l_output="" 
      ...
      if [ "$l_loadable_test" = "passed" ] && [ "$l_loaded_test" = "passed" ] && [ "$l_denied_test" = "passed" ]; then
			...
			echo -e "- End check - Ensure mounting of udf filesystems is disabled" | tee -a "$LOG" 2>> "$ELOG"
   ...
   {
      echo -e "- Start remediation - Ensure mounting of udf filesystems is disabled" | tee -a "$LOG" 2>> "$ELOG"
      ...
      echo -e "- End remediation - Ensure mounting of udf filesystems is disabled" | tee -a "$LOG" 2>> "$ELOG"
   }
